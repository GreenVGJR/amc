module.exports = {
  ...
  code: `
    ...
    $let[nodes;$try[$playerQueueLength[$guildID];0]]
    $onlyIf[$get[nodes]!=0;$callFunction[useCustomMusicMessage;config_errorNoTrackBeforeSeek]]
    $onlyIf[$playerLoopStatus[$guildID]!=track;$callFunction[useCustomMusicMessage;config_errorPlayerBeforeSeek]]

    $if[$option[position]!=;
    $onlyIf[$sub[$option[position];1]<$playerQueueLength[$guildID];$callFunction[useCustomMusicMessage;config_errorPositionBeforeSeek] \`$option[position]\`]
    ]
    ...
    $!playerSkip[$guildID;$option[position]]
    ;
    $!playerSkip[$guildID]
    ]]
