module.exports = {
  ...
  code: `
  $onlyIf[$guildID!=;]
  $onlyIf[$hasPerms[$guildID;$clientID;SendMessages];Missing Permission, **Send Messages** - <@$clientID>]
  $onlyIf[$hasPerms[$guildID;$clientID;Connect];Missing Permission, **Connect** - <@$clientID>]
  $onlyIf[$voiceID!=;You must join a voice channel.]
  $onlyIf[$channelHasPerms[$voiceID;$clientID;Connect];Can't join to voice channel.\nReason: Missing Permission, **Connect** - <@$clientID> (<#$voiceID>)]
  ...
  $let[music_playurl;$if[$get[default_provider]==youtube;https://www.youtube.com/watch?v=$env[result;id];$if[$get[default_provider]==soundcloud;https://soundcloud.com/$env[result;id]]]]
  ;
  ...
  $!editMessage[$channelID;$get[mid];
  $author[Queue;;;0]
  $addField[Added Song;$sub[$queueLength[$guildID];$get[queue_lengthtemp]];true;0]
  $addField[Total Song;$queueLength[$guildID];true;0]
