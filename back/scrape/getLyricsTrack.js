module.exports = {
    ...
    $jsonLoad[pulltrack;$callFunction[filterMediaID;$env[currenttrack;info;uri]]]
    $let[ytmusic;$try[$getYTLyricsMusic[$env[pulltrack;id]];null]]
    ;
    ...
    $arrayLoad[results;]
    $arrayPushJSON[results;$trimLines[{"status_1":null,"status_2":null,"response_time":"$env[ytmusic;ping]","results":{"provider":"youtube","query":"$encodeURI[$env[query]]","url":"$if[$env[isExclude]!=true;$env[currenttrack;info;uri];$env[pullyt;results;0;url]]","autocomplete":"$if[$env[isExclude]!=true;$decodeURI[$env[query]];$env[pullyt;results;0;title]]","lyric":"$deflate[$get[ytmusic];hex]"}}]]
    ;
