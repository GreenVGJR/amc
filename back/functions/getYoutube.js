...
},
{
    ...
    $!httpRequest[https://www.youtube.com/youtubei/v1/search?key=$getCache[authmusic_youtube_key]&prettyPrint=false&fields=header.searchHeaderRenderer.chipBar.chipCloudRenderer.chips.chipCloudChipRenderer(text,navigationEndpoint/continuationCommand/token);POST]
...
},
{
    ...
    $httpAddHeader[User-Agent;$get[agent]]
    $httpAddHeader[Accept-Encoding;gzip]
    $!httpRequest[https://www.youtube.com/youtubei/v1/next?key=$getCache[authmusic_youtube_key]&prettyPrint=false&fields=contents.singleColumnMusicWatchNextResultsRenderer.tabbedRenderer.watchNextTabbedResultsRenderer(tabs.tabRenderer.endpoint.browseEndpoint.browseId);POST;res]
    ...
    $httpSetContentType[Json]
    $httpAddHeader[Accept-Encoding;gzip]
    $!httpRequest[https://www.youtube.com/youtubei/v1/browse?key=$getCache[authmusic_youtube_key]&prettyPrint=false&fields=contents.elementRenderer.newElement.type.componentType.model.timedLyricsModel.lyricsData(timedLyricsData(lyricLine,cueRange(startTimeMilliseconds)));POST]
    ...
    $httpAddHeader[User-Agent;$get[agent]]
    $httpAddHeader[Accept-Encoding;gzip]
    $!httpRequest[https://www.youtube.com/youtubei/v1/browse?key=$getCache[authmusic_youtube_key]&prettyPrint=false&fields=contents.sectionListRenderer.contents.musicDescriptionShelfRenderer.description(runs/text);POST;res2]
    ]]
