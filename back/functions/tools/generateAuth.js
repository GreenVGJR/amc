...
;2];{}]]
    ...
    $try[
        ...
        $if[$get[a14]==;
        $if[$env[successlog]==true;$logger[Warn;Your current session has implement TLS Fingerprinting. Using fallback - Apple Music]]
        $httpAddHeader[User-Agent;$get[agent]]
        $httpAddHeader[Sec-Fetch-Dest;document]
        $httpAddHeader[Sec-Fetch-Site;none]
        ...
        $if[$get[a14]==;
        $if[$env[successlog]==true;$logger[Error;Can't retrieve contents. - Apple Music]]
        $let[blockedapplap;1]
        ]]
    ...
    $if[$and[$get[blockedapplap]!=1;$get[a14]==];$logger[Warn;Re-trying - Apple Music] $callFunction[generateAuth;applemusic;;true]]
    ]
