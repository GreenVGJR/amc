...
;2];{}]]
    ...
    $if[$or[$get[spinitcookies]==;$get[spinitcookies]==undefined]==false;
        ...
        ;
        $jsonLoad[mncsgfsp_ats;$callFunction[generateTotp;,7/*F("rLJ2oxaKL^f+E1xvP@N]]
        $httpAddHeader[User-Agent;$get[agent]]
        $httpAddHeader[Accept-Encoding;gzip, deflate, br]
        $httpAddHeader[Referer;https://open.spotify.com/]
        $httpSetContentType[Text]
        $!httpRequest[https://open.spotify.com/api/token?reason=init&productType=mobile-web-player&totp=$env[mncsgfsp_ats;totp]&totpServer=$env[mncsgfsp_ats;totpServer]&totpVer=61;GET]
        $let[parsejsspo;$httpResult]
        $jsonLoad[parsejsspo;$default[$get[parsejsspo];{}]]
        $let[token;$env[parsejsspo;accessToken]]
        ]
    ...
    $if[$or[$get[spinitcookies]==;$get[spinitcookies]==undefined]==false;
        $jsonLoad[mncsgfsp_atsl;$callFunction[generateTotp;,7/*F("rLJ2oxaKL^f+E1xvP@N]]
        $httpAddHeader[User-Agent;$get[agent]]
        $httpAddHeader[Accept-Encoding;gzip, deflate, br]
        $httpAddHeader[Cookie;$get[spinitcookies]]
        $httpAddHeader[Referer;https://open.spotify.com/]
        $httpSetContentType[Text]
        $!httpRequest[https://open.spotify.com/api/token?reason=init&productType=mobile-web-player&totp=$env[mncsgfsp_atsl;totp]&totpServer=$env[mncsgfsp_atsl;totpServer]&totpVer=61;GET;nkknskdg]
        $let[parsejsspof;$env[nkknskdg]]
        $jsonLoad[parsejsspof;$default[$get[parsejsspof];{}]]
        $let[token2;$env[parsejsspof;accessToken]]
    ]
    $if[$get[token]!=;$setCache[initclientmusic;authmusic_spotify_fall;$get[token]]]
    $if[$get[token2]!=;$setCache[initclientmusic;authmusic_spotify_lyrics;$get[token2]]]
    ...
    $try[
        ...
        $if[$get[a14]!=;$setCache[initclientmusic;authmusic_tiktok_odinid;"$get[a14]"]]
    ...
    $try[
        ...
        $arrayLoad[jfjo;assets/index~;$httpResult]
        $!arrayShift[jfjo]
        $arrayMap[jfjo;kkfl;$return[$advancedTextSplit[$env[kkfl];";0]];jfjo]
        $let[kkfl;$env[jfjo;$arrayFindIndex[jfjo;kfkm;$endsWith[$env[kfkm];.js]]]]
        $httpAddHeader[User-Agent;$get[agent]]
        $!httpRequest[https://music.apple.com/assets/index~$get[kkfl];GET]
        $arrayLoad[noains;eyJ;$httpResult]
        $!arrayShift[noains]
        $arrayMap[noains;kkvl;$return[eyJ$env[kkvl]];noains]
        $let[a14;$advancedTextSplit[$arrayJoin[noains;];";0]]
