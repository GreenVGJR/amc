module.exports = {
    ...
    $let[tryattempt;0]
    $localFunction[refreshsoundcloud;
    $if[$env[retry]==true;$letSum[tryattempt;1]]
    $onlyIf[$get[tryattempt]<3;$return]
    $httpAddHeader[User-Agent;$get[agent]]
    $httpAddHeader[Accept-Encoding;gzip, br]
    $httpSetContentType[Text]
    $let[httpscf;$httpRequest[https://api-v2.soundcloud.com/resolve?url=$get[keturl]&client_id=$getCache[initclientmusic;authmusic_soundcloud];GET;reshttp]]
    $if[$get[httpscf]==429;$callLocalFunction[refreshsoundcloud;true] $return]
    $let[a;$env[reshttp]]
    $return
    ;retry]
    $callLocalFunction[refreshsoundcloud;false]
    ...
    $let[testtempjr;$djsEval[fetch("https://vt.tiktok.com/$env[filterid;id]", { method: "HEAD" }).then(a => a.url).catch()]]
    ...
    $if[$get[ttcounting]>=10;$return]
    $if[$env[retry]==true;$letSum[ttcounting;1]]
    $httpAddHeader[Accept-Encoding;gzip, br]
    $httpAddHeader[Accept-Language;en]
    $httpAddHeader[User-Agent;$get[agent]]
    $httpAddHeader[Cookie;$getCache[initclientmusic;authmusic_tiktok]]
    $httpAddHeader[Referer;https://www.tiktok.com/404]
    $httpAddHeader[Sec-Fetch-Dest;empty]
    $httpAddHeader[Sec-Fetch-Site;same-origin]
    $httpSetContentType[Text]
    $let[http;$httpRequest[https://www.tiktok.com/share/video/$env[filterid;id]?_r=1;GET]]
    ...
    $httpAddHeader[Cookie;$getCache[initclientmusic;authmusic_tiktok]]
    $httpSetContentType[Text]
    $let[http;$httpRequest[https://www.tiktok.com/embed/v2/$env[filterid;id];GET]]
    $if[$or[$get[http]==200;$isJSON[$httpResult]]==false;$return[{"status":null,"results":{"error":"IP blocked"}}]]
    $jsonLoad[a;$advancedTextSplit[$httpResult;type="application/json">;1;</script>;0]]
    $onlyIf[$env[a;source;data;/embed/v2/$env[filterid;id];videoData]!=;$return[{"status":null,"results":{"error":"Content unavailable"}}]]
    $jsonLoad[b;$env[a;source;data;/embed/v2/$env[filterid;id];videoData]]
    ]
    ...
    $!httpRequest[https://api-boot.tiktokv.com/aweme/v1/music/aweme/?music_id=$env[filterid;id]&aid=1233&device_id=$getCache[initclientmusic;authmusic_tiktok_did]&odinId=$getCache[initclientmusic;authmusic_tiktok_odinid]&user_is_login=true&cookie_enabled=true&region=&referer=;GET;a]
    ...
    $httpAddHeader[Accept-Language;en]
    $httpAddHeader[Accept;*/*]
    $httpAddHeader[User-Agent;$callFunction[configMusic;default_userAgent_bot]]
    $httpSetContentType[Text]
    $let[nsdgltt;$httpRequest[https://www.tiktok.com/oembed?url=https://www.tiktok.com/music/-$env[filterid;id];GET;nnoian]]
    $if[$get[nsdgltt]!=200;$return]
    $jsonLoad[nsdglvjon;$env[nnoian]]
    $let[stctitle;$env[nsdglvjon;author_name]]
    $httpAddHeader[Accept-Encoding;gzip, br]
    $httpAddHeader[Accept-Language;en]
    $httpAddHeader[Accept;*/*]
    $httpAddHeader[User-Agent;$get[agent]]
    ...
    $httpAddHeader[X-Khronos;$round[$divide[$getTimestamp;1000]]]
    $httpAddHeader[X-Argus;]
    $!httpRequest[https://api-boot.tiktokv.com/aweme/v1/music/search/?count=10&cursor=0&aid=1988&device_id=$getCache[initclientmusic;authmusic_tiktok_did]&odinId=$getCache[initclientmusic;authmusic_tiktok_odinid]&user_is_login=false&cookie_enabled=true&region=&referer=&keyword=$get[stctitle];GET;c]
    $onlyIf[$env[c]!=;$return]
    $jsonLoad[c;$env[c]]
    ...
    $!djsEval[fetch(ctx.getKeyword("url"), { method: "HEAD", headers: { "Accept": "text/html, */*" } }).then(a => ctx.setKeyword("url", a.url)).catch()]
    $jsonLoad[resolvedfbid;$callFunction[filterMediaID;$get[url]]]
    $httpSetBody[lsd=$getCache[initclientmusic;authmusic_instagram_lsd]&fb_api_caller_class=RelayModern&fb_api_req_friendly_name=PolarisLoggedOutDesktopWWWPostRootContentQuery&server_timestamps=true&variables={"feedLocation":"COMET_MEDIA_VIEWER","privacySelectorRenderLocation":"COMET_MEDIA_VIEWER","renderLocation":"comet_media_viewer","nodeID":"$default[$env[resolvedfbid;id];$env[filterid;id]]","scale":1,"shouldShowComments":false}&doc_id=28067965802883859]
    $httpAddHeader[Accept;*/*]
    $httpAddHeader[Content-Type;application/x-www-form-urlencoded]
    $httpAddHeader[Accept-Encoding;gzip, br]
    $httpAddHeader[Sec-Fetch-Site;same-origin]
    $httpAddHeader[X-ASBD-ID;359341]
    $httpAddHeader[X-FB-LSD;$getCache[initclientmusic;authmusic_instagram_lsd]]
    $httpAddHeader[User-Agent;$get[agent]]
    $httpSetContentType[Text]
    $let[uynsdfp;$httpRequest[https://www.facebook.com/api/graphql;POST;noiio]]
    $arrayLoad[kka;
;$env[noiio]]
    $let[results;{"status":null,"results":$if[$typeof[$env[kka;0]]==object;$env[kka;0];null]}]
    ]
    $if[$env[filterid;type]==instagram;
    $c[Instagram GraphQL Api]
    $httpSetBody[lsd=$getCache[initclientmusic;authmusic_instagram_lsd]&fb_api_caller_class=RelayModern&fb_api_req_friendly_name=PolarisLoggedOutDesktopWWWPostRootContentQuery&server_timestamps=true&variables={"media_id":"$callFunction[codeToIdInstagram;$advancedTextSplit[$env[filterid;id];/;$charCount[$env[filterid;id];/]]]"}&doc_id=27130156389949648]
    $httpAddHeader[Accept;*/*]
    $httpAddHeader[Content-Type;application/x-www-form-urlencoded]
    $httpAddHeader[Accept-Encoding;gzip, br]
    $httpAddHeader[Sec-Fetch-Site;same-origin]
    $if[$getCache[initclientmusic;authmusic_instagram_csrf]!=;
    $httpAddHeader[X-CSRFToken;$getCache[initclientmusic;authmusic_instagram_csrf]]
    ]
    $httpAddHeader[X-ASBD-ID;359341]
    $httpAddHeader[X-FB-LSD;$getCache[initclientmusic;authmusic_instagram_lsd]]
    $httpAddHeader[X-IG-App-ID;$default[$getCache[initclientmusic;authmusic_instagram_appid];936619743392459]]
    $httpAddHeader[User-Agent;$get[agent]]
    $httpSetContentType[Text]
    $let[uynsdfp;$httpRequest[https://www.instagram.com/api/graphql;POST]]
    $if[$typeof[$httpResult]==object;$jsonLoad[lck;$httpResult]]
    $if[$or[$env[lck]==;$env[lck;data;require_login]==true;$get[uynsdfp]==403;$get[uynsdfp]==429];
    $c[Instagram Embed Webpage]
    ...
    $if[$or[$get[uynsdf]==403;$get[uynsdf]==429];
    $c[Instagram Webpage]
    $httpAddHeader[Accept;text/html, */*]
    $httpAddHeader[Accept-Language;en]
    $httpAddHeader[Accept-Encoding;gzip, br]
    ...
    ;
    $jsonLoad[lck;$default[$env[lck;data;xig_polaris_media;if_not_gated_logged_out];{}]]
    ]
    ...
    $if[$advancedTextSplit[$get[url];/share/;1]!=;
    $let[sharelk;$get[url]]
    $let[resolvedlk;$djsEval[fetch(ctx.getKeyword("sharelk")).then(a=>a.url).catch()]]
    $jsonLoad[filterid;$callFunction[filterMediaID;$get[resolvedlk]]]
    ]
    $httpAddHeader[Accept;text/html, */*]
    $httpAddHeader[Accept-Language;en]
    $httpAddHeader[Accept-Encoding;gzip, br]
    ...
    $!jsonSet[rty;postid;$env[filterid;id]]
    $arrayLoad[thimgs]
    $arrayLoad[thraw;MediaScrollImageContainer"><img class="img" src=";$httpResult]
    $arraySlice[thraw;thraw;1;10]
    $arrayForEach[thraw;thim;$arrayPush[thimgs;$djsEval[require("entities").decodeHTML(ctx.getEnvironmentKey("thim").split('"')[0\\])]]]
    $arrayPush[thimgs;$advancedTextSplit[$httpResult;og:image" content=";1;";0]]
    $!jsonSet[rty;images;$env[thimgs]]
