module.exports = {
    ...
    $httpSetBody[{"sdk_version":"100.0.0","count":10,"cursor":"0","query":"$replace[$replace[$env[query];\\\\;];";\\\\"]","scene":1,"search_version":2}]
    $httpSetContentType[Text]
    $httpAddHeader[Content-Type;application/json]
    $httpRemoveHeader[Accept-Encoding]
    $httpAddHeader[User-Agent;$get[agent]]
    $httpAddHeader[Sign;$md5[9e2c|mplates|7|5.8.0|$get[time]||11ac]]
    ...
    $!httpRequest[https://edit-api-sg.capcut.com/lv/v1/cc_web/replicate/search_templates;POST]
    $if[$isJSON[$httpResult]]==false;$return]
    $jsonLoad[r;$httpResult]
    $jsonLoad[r;$env[r;data;video_templates]]
    $arrayForEach[r;tp;$arrayPushJSON[results;{"title":"$replace[$replace[$default[$env[tp;short_title];$env[tp;title]];\\\\;];";\\\\"]","duration":"$parseDigital[$env[tp;duration]]","thumbnail":"$env[tp;cover_url]","url":"https://capcut.com/templates/$env[tp;web_id]"}]]
    ]
    ...
    $if[$env[l;0]==;$return]
    $arrayForEach[l;tp;$arrayPushJSON[results;{"title":"$replace[$replace[$env[tp;translation;description];\\\\;];";\\\\"]","duration":"$env[tp;duration]","thumbnail":"$env[tp;imagePath]","url":"https://www.kinemaster.com/kinespace/detail/$env[tp;projectId]"}]]
    ]
