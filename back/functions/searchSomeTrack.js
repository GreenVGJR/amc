module.exports = {
    ...
    $let[agent;Whee 1.0.1 rv:10102 (iPhone\\; iOS 16.2\\; en_US) Cronet]
    $httpSetContentType[Text]
    $httpAddHeader[Accept-Encoding;]
    $httpAddHeader[Accept-Language;en]
    $httpAddHeader[User-Agent;$get[agent]]
    $httpAddHeader[Cookie;$inflate[$getCache[authmusic_tiktok];base64]]
    $httpAddHeader[X-Gnarly;$callFunction[xGnarlyTiktok;count=10&keyword=$encodeURI[$env[query]]&version_code=3.2.0&app_name=musical_ly&channel=App+Store&device_id=$getCache[authmusic_tiktok_did]&aid=1180&os_version=16.2&device_platform=iphone&iid=7386407102867523334&device_brand=iphone&device_type=iPhone10,6;$get[agent]]]
    $httpAddHeader[X-Khronos;$round[$divide[$getTimestamp;1000]]]
    $httpAddHeader[X-Argus;]
    $!httpRequest[https://api-boot.tiktokv.com/aweme/v1/search/item/?count=10&keyword=$encodeURI[$env[query]]&version_code=3.2.0&app_name=musical_ly&channel=App+Store&device_id=$getCache[authmusic_tiktok_did]&aid=1180&os_version=16.2&device_platform=iphone&iid=7386407102867523334&device_brand=iphone&device_type=iPhone10,6;GET;a]
    $if[$env[a]==;$return]
    $jsonLoad[a;$env[a]]
    ...
    $!jsonSet[kls;duration;$parseDigital[$env[d;video;duration]]]
    $!jsonSet[kls;thumbnail;$env[d;video;cover;url_list;0]]
    $!jsonSet[kls;url;https://www.tiktok.com/@$env[d;author;unique_id]/video/$env[d;aweme_id]]
    $arrayPush[results;$jsonStringify[kls]]
    ]
    ]
    ...
    $!jsonSet[kls;title;$env[d;music;title]]
    $!jsonSet[kls;duration;$parseDigital[$multi[$env[d;music;duration];1000]]]
    $!jsonSet[kls;thumbnail;$env[d;music;cover_hd;url_list;0]]
    $!jsonSet[kls;url;https://www.tiktok.com/music/-$env[d;music;id_str]]
    $arrayPush[results;$jsonStringify[kls]]
    ]
    ]
    ...
    $if[$env[provider]==beatport;
    $httpAddHeader[Accept-Encoding;]
    $httpAddHeader[User-Agent;$get[agent]]
    $httpSetContentType[Text]
    $!httpRequest[https://www.beatport.com/search/tracks?q=$env[query];GET]
    $let[kflhttp;$advancedTextSplit[$httpResult;NEXT_DATA;1;type="application/json">;1;</script>;0]]
    $if[$get[kflhttp]==;$return]
    $jsonLoad[a;$get[kflhttp]]
    $jsonLoad[a;$env[a;props;pageProps;dehydratedState;queries;0;state;data;data]]
    $arraySlice[a;a;0;10]
    $arrayForEach[a;tp;
    $jsonLoad[kls;{}]
    $!jsonSet[kls;title;$env[tp;track_name]]
    $!jsonSet[kls;duration;$parseDigital[$env[tp;length]]]
    $!jsonSet[kls;thumbnail;$replace[$env[tp;release;release_image_dynamic_uri];{w}x{h};2160x2160]]
    $!jsonSet[kls;url;https://www.beatport.com/track/$encodeURI[$env[tp;artists;0;artist_name]]/$env[tp;track_id]]
    $arrayPush[results;$jsonStringify[kls]]
    ]
    ]
    $if[$env[provider]==facebookreels;
    $httpAddHeader[Accept-Encoding;]
    $httpAddHeader[Accept;text/html]
    $httpAddHeader[Sec-Fetch-Site;none]
    $httpAddHeader[User-Agent;$get[agent]]
    $httpSetContentType[Text]
    $!httpRequest[https://web.facebook.com/watch/search/?q=$env[query];GET;f]
    $arrayLoad[g;script type="application/json";$env[f]]
    $!arrayShift[g]
    $jsonLoad[vr;$default[$advancedTextSplit[$env[g;$arrayFindIndex[g;h;$checkCondition[$advancedTextSplit[$env[h];searchCometResultsNormalVideo;1]!=]]];data-sjs>;1;</script>;0];{}]]
    $if[$env[vr;require;0;3;0;__bbox;require;5;3;1;__bbox;result;data;serpResponse;results;edges;0]==;$return]
    $jsonLoad[vr;$env[vr;require;0;3;0;__bbox;require;5;3;1;__bbox;result;data;serpResponse;results;edges]]
    $arrayForEach[vr;py;
    $jsonLoad[kls;{}]
    $!jsonSet[kls;title;$env[py;rendering_strategy;view_model;video_metadata_model;title]]
    $!jsonSet[kls;duration;$parseDigital[$unparseDigital[$env[py;rendering_strategy;view_model;video_thumbnail_model;video_duration_text]]]]
    $!jsonSet[kls;thumbnail;$env[py;rendering_strategy;view_model;video_thumbnail_model;thumbnail_image;uri]]
    $!jsonSet[kls;url;https://web.facebook.com$env[py;rendering_strategy;view_model;video_click_model;click_metadata_model;payload;open_video_uri]]
    $arrayPush[results;$jsonStringify[kls]]
    ]
    ]
    ]
