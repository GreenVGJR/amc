module.exports = {
    ...
    $arrayForEach[res2;res3;$arrayPushJSON[results;{"title":"$replace[$replace[$env[res3;trackName];\\\\;];";\\\\"]","duration":"$parseDigital[$env[res3;trackTimeMillis]]","thumbnail":"$replace[$env[res3;artworkUrl100];100x100bb;1x1ss]","url":"https://music.apple.com/us/song/$advancedTextSplit[$env[res3;trackViewUrl];/;$sub[$charCount[$env[res3;trackViewUrl];/];1]]/$env[res3;trackId]"}]]
    ]
    ...
    $arrayForEach[res2;res5;$arrayPushJSON[results;{"title":"$replace[$replace[$env[res5;attributes;name];\\\\;];";\\\\"]","duration":"$parseDigital[$env[res5;attributes;durationInMillis]]","thumbnail":"$replace[$env[res5;attributes;artwork;url];{w}x{h}bb;1x1ss]","url":"https://music.apple.com/us/song/$advancedTextSplit[$env[res5;attributes;url];/;$sub[$charCount[$env[res5;attributes;url];/];1]]/$env[res5;id]"}]]
    ]
    ...
    $arrayForEach[res;tp;$arrayPushJSON[results;{"title":"$replace[$replace[$env[tp;attributes;name];\\\\;];";\\\\"]","duration":"$parseDigital[$env[tp;attributes;durationInMillis]]","thumbnail":"$replace[$env[tp;attributes;artwork;url];{w}x{h}bb;1x1ss]","url":"https://www.shazam.com/song/$env[tp;id]/$advancedTextSplit[$env[tp;attributes;url];/;$sub[$charCount[$env[tp;attributes;url];/];1]]"}]]
    ]
    ...
    $arrayForEach[res;tp;$arrayPushJSON[results;{"title":"$replace[$replace[$env[tp;song];\\\\;];";\\\\"]","duration":"$parseDigital[$multi[$env[tp;duration];1000]]","thumbnail":"$env[tp;image]","url":"$default[$env[tp;perma_url];$env[tp;album_url]]"}]]
    ]
    ]
    $if[$env[results;0]!=;
    $jsonLoad[lf;{}]
